document.addEventListener('DOMContentLoaded', async () => {
    // 1. Seleccionar los contenedores (Ajustados para evitar errores null)
    const galeria = document.getElementById('contenedor-galeria');
    const heroTexto = document.querySelector('.textohero');
    const heroForm = document.querySelector('.hero form'); // Ajustado el selector
        ...
        if (galeria) {
            if (data.productos && data.productos.length > 0) {
                ...
                data.productos.forEach(prod => {
                    ...
                    // Se añade "/" al inicio de la ruta para asegurar que sea desde la raíz en Vercel
                    galeria.innerHTML += `
                        <div class="producto">
